...
function LogMessageCell({ m }) {
  const type = m.type || 'text';

  if (type === 'image') {
    if (!m.media_url) {
      return <span className="italic text-amber-600 dark:text-amber-400">Image — no URL stored</span>;
    }
    ...
    return (
      <div className="space-y-2 py-1">
        <a
          ...
          className="inline-block rounded-lg border border-gray-200 dark:border-gray-600 overflow-hidden bg-gray-100 dark:bg-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500"
        >
          <img
            src={m.media_url}
            alt={caption || 'Chat image'}
            className="max-h-48 max-w-[min(18rem,70vw)] w-auto object-contain"
            loading="lazy"
          />
        </a>
  ...
  if (type === 'voice') {
    return (
      <div className="space-y-1 py-1">
        {m.media_url ? (
          <audio controls src={m.media_url} className="h-9 w-full max-w-xs" preload="metadata" />
        ) : (
          <span className="italic text-amber-600 dark:text-amber-400">Voice — no URL</span>
        )}
      </div>
    );
  }
  ...
  if (type === 'location') {
    return (
      <div className="space-y-1 text-sm py-1">
        {m.location_address ? (
          <p className="text-gray-700 dark:text-gray-300">{m.location_address}</p>
        ) : null}
        {m.location_lat != null && m.location_lng != null ? (
          <p className="font-mono text-xs text-gray-500 dark:text-gray-400">
            {Number(m.location_lat).toFixed(5)}, {Number(m.location_lng).toFixed(5)}
          </p>
        ) : null}
        {m.text?.trim() ? <p className="text-gray-600 dark:text-gray-400">{m.text}</p> : null}
        {!m.location_address && (m.location_lat == null || m.location_lng == null) ? (
          <span className="italic text-gray-400">No location data</span>
        ) : null}
      </div>
    );
  }
  ...
  return (
    <span className="line-clamp-6 whitespace-pre-wrap break-words text-gray-700 dark:text-gray-300">
      {m.text?.trim() ? m.text : '—'}
    </span>
  );
}
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      worksheet.columns = [
        ...
        { header: 'Media / location', key: 'extra', width: 45 },
      ...
      (data || []).forEach((m) => {
        const t = m.type || 'text';
        let extra = '';
        if (t === 'image' || t === 'voice') extra = m.media_url || '';
        else if (t === 'location') {
          extra = [m.location_address, m.location_lat != null && m.location_lng != null ? `${m.location_lat},${m.location_lng}` : '']
            .filter(Boolean)
            .join(' | ');
        }
        worksheet.addRow({
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-soft border border-gray-100 dark:border-gray-700 p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            <input
              ...
              className="flex-1 min-w-[200px] rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-4 py-2 text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-teal-500 focus:border-transparent transition"
            />
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-4 py-2 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent transition"
            />
            <input
              type="date"
              value={endDate}
            ...
            <button
              ...
              className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl transition text-sm font-medium shadow-sm"
            >
              Apply
            </button>
            <button
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-soft border border-gray-100 dark:border-gray-700 overflow-hidden">
          {loading ? (
            <div className="flex justify-center py-16">
              <BrandedLoader message="Loading chat logs…" size="md" />
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-900">
                    <tr>
                      {['Sender', 'Type', 'Message', 'Time', 'Expires', 'Critical'].map((h) => (
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {messages.map((m) => (
                      <tr key={m.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700 transition ${m.is_critical ? 'bg-red-50/40 dark:bg-red-900/10' : ''}`}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 max-w-md min-w-[12rem] align-top">
                          <LogMessageCell m={m} />
                        </td>
