...
function ModerationCard({ incident, onApprove, onReject, processing }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm">
      ...
      {/* Structured evidence — same grouping as the incident report form */}
      <div className="mb-4">
          ...
          switch (payload.eventType) {
            ...
            case 'UPDATE':
              ...
              if (payload.new.status !== 'pending') {
                ...
                // Still pending, update row but keep structured evidence list
                setIncidents(prev => prev.map(i =>
                  i.id === payload.new.id ? { ...payload.new, evidenceRows: i.evidenceRows || [] } : i
                ));
              }
              break;
              ...
              if (!processedIds.current.has(payload.new.id)) {
                void (async () => {
                  const { data: ev } = await supabase
                    .from("incident_evidence")
                    .select("*")
                    .eq("incident_id", payload.new.id)
                    .order("created_at", { ascending: true });
                  setIncidents((prev) => [
                    { ...payload.new, evidenceRows: ev || [] },
  ...
  if (loading && incidents.length === 0) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        <BrandedLoader message="Loading pending incidents…" size="lg" />
      </div>
    );
  }
