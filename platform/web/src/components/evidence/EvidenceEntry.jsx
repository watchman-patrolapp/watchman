...
import { MAX_IMAGES_PER_EVIDENCE_ENTRY } from '../../utils/evidenceHelpers';
...
export default function EvidenceEntry({ 
  ...
  const handleFileChange = (e) => {
    ...
    const current = entry.files?.length || 0;
    if (current + selectedFiles.length > MAX_IMAGES_PER_EVIDENCE_ENTRY) {
      alert(`Maximum ${MAX_IMAGES_PER_EVIDENCE_ENTRY} images allowed per entry`);
      return;
    }
  ...
  const UploadIcon = entry.category === 'scene_photos' ? FaCamera : FaImage;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-600 shadow-sm">
      ...
      <div className="mb-3">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          {entry.files?.length > 0 ? (
            <span className="text-gray-500 dark:text-gray-400 font-normal"> (optional if you add photos)</span>
          ) : (
        ...
        <textarea
          ...
          placeholder={
            entry.files?.length > 0
              ? 'Optional: e.g. person seen near scene, not stopped — add whatever helps analysts.'
              : 'Describe what this shows, when it was taken, and its relevance…'
          }
          rows={2}
      ...
      <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4 hover:border-teal-500 dark:hover:border-teal-400 transition">
        ...
        >
          <UploadIcon className="w-8 h-8 text-gray-400 mb-2" aria-hidden />
          <span className="text-sm text-gray-600 dark:text-gray-400 text-center">
            {entry.files?.length > 0
              ? `${entry.files.length} image(s) selected (max ${MAX_IMAGES_PER_EVIDENCE_ENTRY})`
              : `Drop images here or click to upload (max ${MAX_IMAGES_PER_EVIDENCE_ENTRY})`}
          </span>
        </label>
