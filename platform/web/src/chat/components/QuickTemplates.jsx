  ...
  const handleTemplateClick = useCallback(
    async (template) => {
      ...
      try {
        ...
        // Request GPS in the same synchronous chain as this click (before audio / other awaits)
        if (template.requireLocation) {
          try {
            ...
            if (template.category === 'emergency' && template.requireLocation) {
              toast.error(
                'Location required for this message. Allow location for this site, or use Attach → Manual coordinates.',
                { duration: 6000 }
              );
              return;
            }
