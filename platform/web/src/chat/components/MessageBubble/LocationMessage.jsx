...
export const LocationMessage = React.memo(function LocationMessage({ 
  ...
  // PostgREST / JSON may return numeric columns as strings
  const latNum = lat != null && lat !== '' ? Number(lat) : NaN;
  const lngNum = lng != null && lng !== '' ? Number(lng) : NaN;
  const isValidLocation = isValidCoordinate(latNum, lngNum);
  ...
  // Lazy load map when this message's map area enters the viewport (not a global .leaflet-container query)
  useEffect(() => {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border max-w-[320px] ${
      ...
      {isValidLocation ? (
        <div className="relative z-0 isolate overflow-hidden" ref={mapAreaRef}>
          ...
          {!mapError && isMapVisible ? (
            <div className="relative h-36 w-full">
              ...
              <a 
                ...
                className="absolute inset-0 z-20 flex items-center justify-center bg-black/0 hover:bg-black/10 transition-colors group min-h-[44px] min-w-[44px]"
          ...
          <div 
            className="pointer-events-none absolute bottom-2 left-2 z-30 rounded bg-black/70 px-2 py-1 font-mono text-xs text-white"
