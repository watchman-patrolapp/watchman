...
export class RateLimiter {
  ...
  /**
   * Read-only check for UI (e.g. disabling the composer). Does not consume a slot.
   * `canProceed()` must only be called when actually attempting an action — it records each call.
   */
  isRateLimited() {
    const now = Date.now();
