...
async function safeCleanup(
  ...
  if (error) console.error(`admin-delete-user cleanup [${label}]:`, error.message || error)
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!serviceRoleKey) {
      console.error('admin-delete-user: missing SUPABASE_SERVICE_ROLE_KEY (or SERVICE_ROLE_KEY)')
    ...
    if (callerRole !== 'admin' && callerRole !== 'technical_support') {
      return new Response(JSON.stringify({ error: 'Admin or technical support role required' }), {
        status: 403,
        headers: { ...headers, 'Content-Type': 'application/json' },
      })
    }
    ...
    await safeCleanup('criminal_profiles created_by', () =>
      supabaseAdmin.from('criminal_profiles').delete().eq('created_by', targetId)
    )

    const { error: delErr } = await supabaseAdmin.auth.admin.deleteUser(targetId)
    // Auth user may already be gone (e.g. deleted in dashboard) while public.users still has a row.
    ...
    if (profileErr) {
      console.error('admin-delete-user: public.users delete failed:', profileErr.message || profileErr)
      return new Response(
