-- Residents list includes any linked security company so partners can
-- see their own clients vs other companies vs unlinked households.
...
AS $$
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE mm.resident_user_id = u.id
      AND coalesce(mm.membership_status, '') NOT IN ('rejected', 'expired')
    ORDER BY
