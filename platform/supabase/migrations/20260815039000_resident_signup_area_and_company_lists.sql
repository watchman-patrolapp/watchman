-- Public signup lists (areas + security companies) and resident join-by-dropdown.
-- Anonymous users cannot read organizations via RLS, so expose a narrow RPC.
...
COMMENT ON FUNCTION public.list_public_signup_options() IS
  'Public register form options: active neighborhood areas and listed security companies.';
...
BEGIN
  ...
  IF track = 'security_company' THEN
    ...
    -- Listed companies only. Do not create a new company from the public form.
