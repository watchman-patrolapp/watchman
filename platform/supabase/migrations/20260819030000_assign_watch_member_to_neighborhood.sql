-- Allow assign-to-suburb for watch roles (patroller, etc.), not only households.
...
COMMENT ON FUNCTION public.assign_resident_to_neighborhood(uuid, uuid) IS
  'Staff places a household or watch member on a neighborhood watch and its primary suburb.';
