...
export default function WatchlistTemplatePicker({ watchlistFlags, setWatchlistFlags }) {
  return (
    <div className="mt-3 rounded-lg border border-dashed border-amber-200 bg-amber-50/50 p-3 dark:border-amber-900/40 dark:bg-amber-950/20">
      <p className="mb-2 text-xs font-medium text-amber-900 dark:text-amber-200">Quick-add watchlist tags</p>
      <p className="mb-3 text-xs text-amber-800/90 dark:text-amber-300/80">
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
        {WATCHLIST_FLAG_TEMPLATE_GROUPS.map((group) => (
          <details
            key={group.id}
            className="group rounded-md border border-amber-200/80 bg-white dark:border-amber-900/50 dark:bg-gray-900/40"
          >
            ...
            <div className="flex flex-wrap gap-1.5 border-t border-amber-100 px-3 py-2 dark:border-amber-900/40">
              {group.flags.map((flag) => (
                <button
                  key={flag}
                  type="button"
                  onClick={() => setWatchlistFlags((prev) => appendWatchlistFlag(prev, flag))}
                  className="inline-flex items-center gap-1 rounded-full border border-amber-300/80 bg-white px-2 py-1 text-left text-xs text-amber-950 hover:bg-amber-100 dark:border-amber-800 dark:bg-gray-800 dark:text-amber-100 dark:hover:bg-gray-700"
                >
