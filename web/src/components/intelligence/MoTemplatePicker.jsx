...
export default function MoTemplatePicker({ moSignature, setMoSignature }) {
  return (
    <div className="mb-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3 dark:border-slate-600 dark:bg-slate-900/30">
      <p className="mb-2 text-xs font-medium text-slate-800 dark:text-slate-200">MO starter templates</p>
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
        {MO_TEMPLATE_GROUPS.map((group) => (
          <details
            key={group.id}
            className="group rounded-md border border-slate-200 bg-white dark:border-slate-600 dark:bg-gray-900/50"
          >
            <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2 text-xs font-semibold text-gray-800 dark:text-gray-200 [&::-webkit-details-marker]:hidden">
              {group.title}
              <FaChevronDown className="h-3 w-3 shrink-0 text-slate-500 transition group-open:rotate-180" />
            </summary>
            <ul className="space-y-1 border-t border-slate-100 px-2 py-2 dark:border-slate-700">
